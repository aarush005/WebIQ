...
const handleAudit = async ()=>{
  ...
  const ticker = setInterval(()=>{
    i = Math.min(i + 1, Parentheses.length - 1);
    setPhaseIdx(i);
  }, 2000);

  await analyze(url.trim());
  clearInterval(ticker);
  navigate("/dashboard"); // go to result page
}
...
return (
  <main className='min-h-screen bg-[#0f0f1a] flex flex-col items-center justify-center px-4'>
    <div className="max-w-2xl w-full text-center">
      ...
      <input type="url"
      value={url}
      onChanbe={e => setUrl(e.target.value)}
      ...
      {/* Feature pills */}
      <div className="flex flex-wrap gap-2 justify-centerr-mt-12">
