...
export default function Dashboard() {
  ...
  const overallColor =
    current.overallScore >= 70 ? "#16a34a" :
    current.overallScore >= 50 ? "#ea580c" : "#c0392b";
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-5xl mx-auto px-4 py-8">
        ...
        <div className="flex items-center gap-4 mb-6 flex-wrap">
          <button
            onClick={() => navigate("/")}
            className="bg-white border border-gray-200 text-gray-600 px-4 py-2 rounded-xl text-sm hover:bg-gray-50 transition"
          >
            ← New Audit
          </button>

          <div className="flex-1 min-w-0">
            <p className="font-bold text-gray-900 text-lg truncate">{current.url}</p>
            <p className="text-sm text-gray-400">
              Stack: <strong className="text-gray-600">{current.stack || "Unknown"}</strong>
              {current.auditedAt && (
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="text-center">
              <ScoreRing score={current.overallScore} color={overallColor} size={64} />
          ...
          {activeTab === "competitor" && current.competitor && (
            <div>
              <div className="flex items-center gap-4 mb-5">
                ...
                <div>
                  <p className="font-semibold text-gray-900 text-base">Competitor Analysis</p>
                  <p className="text-sm text-gray-400">
                    Tracking: {current.competitor.competitors?.join(", ") || "N/A"}
                  </p>
                </div>
              </div>
                ...
                <div className="mb-5">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {current.competitor.keywords.map((k, i) => (
                      <span key={i} className="bg-purple-50 text-purple-700 border border-purple-200 rounded-full px-3 py-1 text-sm">
                        {k}
                      </span>
                    ))}
                  </div>
